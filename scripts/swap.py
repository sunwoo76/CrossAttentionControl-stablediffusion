...
def load_img(path):
    ...
    w, h = 512, 512#map(lambda x: x - x % 32, (w, h))  # resize to integer multiple of 32
...
def load_mask(path):
    ...
    w, h = 512//8, 512//8#map(lambda x: x - x % 32, (w, h))  # resize to integer multiple of 32
...
def main():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="models/ldm/stable-diffusion-v1/sd-v1-3-full-ema.ckpt",
    ...
    if opt.init_img is not None:
        ...
        init_image = repeat(init_image, '1 ... -> b ...', b=batch_size) # torch.Size([2, 3, 512, 512])

    if opt.use_mask and opt.init_mask is not None:
        init_mask = load_mask(opt.init_mask).to(device) # torch.Size([1, 3, 512, 512])
        init_mask = repeat(init_mask, '1 ... -> b ...', b=batch_size) # torch.Size([2, 3, 512, 512])
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        if opt.scale != 1.0: # default  = 5.0
                            uc = model.get_learned_conditioning(batch_size * [""]) # uc.shape (2, 77, 768)
                        ...
                        if init_latent is not None:
                            ...
                            if opt.fixed_code:
                                start_code = torch.Tensor(np.load("./z_fixed.npy")).float().to(device=device)
                                z_enc=start_code[0:1].repeat(3,1,1,1)
                                sz_enc=start_code[0:1].repeat(3,1,1,1)
                            else:
                                z_enc = torch.randn((batch_size,4,64,64)).to(device)
                                sz_enc = torch.empty_like(z_enc).copy_(z_enc).to(device) #torch.Tensor(z_enc).to(device)
                        # decode it
                        ...
                        if not opt.is_swap:
                            if not opt.use_mask:
                                ...
                                samples = sampler.decode(z_enc, c, t_enc, x0=init_latent_bg, mask=init_mask, unconditional_guidance_scale=opt.scale,
                                                        unconditional_conditioning=uc,)
                        else:
                            pmask = get_indice(model, prompts, sprompts) #.to(device)
                            if not opt.use_mask:
                                samples, s_samples = sampler.decode(z_enc, c, t_enc, sx_latent=sz_enc, scond=sc, pmask=pmask, unconditional_guidance_scale=opt.scale,
                                                    unconditional_conditioning=uc,)
                            else:
                                samples, s_samples = sampler.decode(z_enc, c, t_enc, sx_latent=sz_enc,  scond=sc, pmask=pmask, x0=init_latent_bg, mask=init_mask, unconditional_guidance_scale=opt.scale,
                                                        unconditional_conditioning=uc,)

                        x_samples = model.decode_first_stage(samples) # (2,3,512,512), imgsize=256 기준., # swap된 sample.
                        x_samples = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)

                        if opt.is_swap:
                            sx_samples = model.decode_first_stage(s_samples) # (2,3,512,512), imgsize=256 기준.
                            sx_samples = torch.clamp( (sx_samples + 1.0) / 2.0, min=0.0, max=1.0)
                        ...
                        if not opt.skip_save:
                            ...
                            if opt.is_swap:
                                ...
                                for sx_sample in sx_samples:
                                    sx_sample = 255. * rearrange(sx_sample.cpu().numpy(), 'c h w -> h w c')
                ...
                if not opt.skip_grid:
                    ...
                    Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, prompts[0].replace(" ", "_") + "_tar.png"))
                    ...
                    if opt.is_swap:
                        ...
                        # to image
                        sgrid = 255. * rearrange(sgrid, 'c h w -> h w c').cpu().numpy()
                        #Image.fromarray(grid.astype(np.uint8)).save(os.path.join(outpath, f'grid-{grid_count:04}.png'))
                        Image.fromarray(sgrid.astype(np.uint8)).save(os.path.join(outpath, sprompts[0].replace(" ", "_") + "_src.png"))
